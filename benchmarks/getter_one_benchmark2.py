"""
benchmark — GETTER One (adaptive) vs baselines
"""
...
def generate_bidirectional(cfg, seed):
    """S3: 双方向因果 A→B(lag=2) かつ B→A(lag=3)"""
    rng=_rng(seed); T=cfg.T+cfg.burn_in; x=np.zeros((T,3))
    for t in range(1,T):
        # A→B(lag=2) かつ B→A(lag=3)
        ...
        x[t,2]=0.60*x[t-1,2]+rng.normal(scale=cfg.noise_scale)  # independent
    x=x[cfg.burn_in:]; names=["A","B","Noise"]
...
def generate_chain(cfg, seed):
    """S4: 因果連鎖 A→B(lag=2)→C(lag=2)、AとCに直接因果なし"""
    rng=_rng(seed); T=cfg.T+cfg.burn_in; x=np.zeros((T,3))
    for t in range(1,T):
        x[t,0]=0.70*x[t-1,0]+rng.normal(scale=cfg.noise_scale)
...
# ── GETTER One Adapter (v0.1.8 FIXED!) ──
...
class GetterOneAdapter:
    ...
    def fit(self, df, cfg):
        ...
        config = PipelineConfig(
            ...
            enable_anomaly=True,
            enable_extended=True,  # ベンチマーク不要 → 高速化
            enable_phase_space=True,  # ベンチマーク不要 → 高速化
            enable_network=True,
            sync_threshold=self.sync_threshold,
            causal_threshold=self.causal_threshold,
            max_lag=self.max_lag,
            adaptive_network=True,  # 🔥 NEW! データ駆動閾値
        ...
        return MethodOutput(
            ...
            lag_matrix=lagm,
            sign_matrix=signm,
            meta={"pattern": result.pattern, "adaptive": result.adaptive_params},
        )
...
class VARGrangerAdapter:
    ...
    def __init__(self, maxlags=8, alpha=0.05):
    ...
    def fit(self, df, cfg):
        ...
        with warnings.catch_warnings():
            ...
            model = VAR(df.copy())
            results = model.fit(
                maxlags=min(self.maxlags, max(1, len(df) // 10)), ic="aic"
            )
        scores = np.zeros((n, n))
        ...
        for i, src in enumerate(names):
            for j, dst in enumerate(names):
                ...
                adj[i, j] = int(pval < self.alpha)
                cv, cl = [], []
                for lag in range(1, results.k_ar + 1):
                    rn = f"L{lag}.{src}"
                    if rn in params.index and dst in params.columns:
                        cv.append(float(params.loc[rn, dst]))
                        cl.append(lag)
                if cv:
                    idx = int(np.argmax(np.abs(cv)))
                    lagm[i, j] = cl[idx]
                    signm[i, j] = int(np.sign(cv[idx]))
...
class TransferEntropyAdapter:
    ...
    def __init__(self, max_lag=8, n_perm=30):
        self.max_lag = max_lag
        self.n_perm = n_perm
            ...
            for j in range(n):
                ...
                for lag in range(1, min(self.max_lag, len(df) - 2) + 1):
                    ...
                    if te > best_te:
                        ...
                        best_lag = lag
                null = [
                    _te_discrete(rng.permutation(x[:, i]), x[:, j], lag=best_lag)
                    for _ in range(self.n_perm)
                ]
                pval = (1 + np.sum(np.array(null) >= best_te)) / (self.n_perm + 1)
                scores[i, j] = max(best_te, 0)
                lagm[i, j] = best_lag
                adj[i, j] = int(pval < 0.05)
        np.fill_diagonal(scores, 0)
        np.fill_diagonal(adj, 0)
        return MethodOutput(
            method_name=self.method_name,
...
class GraphicalLassoAdapter:
    ...
    def __init__(self, edge_thr=0.03):
    ...
    def fit(self, df, cfg):
        ...
        with warnings.catch_warnings():
            warnings.simplefilter("ignore")
            m = GraphicalLassoCV()
            m.fit(StandardScaler().fit_transform(df.to_numpy()))
        p = np.abs(m.precision_)
        np.fill_diagonal(p, 0)
        adj = (
            (p >= self.edge_thr).astype(int) + ((p >= self.edge_thr).astype(int)).T > 0
        ).astype(int)
        np.fill_diagonal(adj, 0)
        return MethodOutput(
            method_name=self.method_name,
    ...
    def fit(self, df, cfg):
        ...
        for i in range(n):
            for j in range(n):
                ...
                bs, bl = -np.inf, 1
                for lag in range(1, min(self.max_lag, len(df) - 2) + 1):
                    s = _event_sync(events[:, i], events[:, j], lag)
                    if s > bs:
                        bs = s
                        bl = lag
                null = [
                    _event_sync(rng.permutation(events[:, i]), events[:, j], lag=bl)
                    for _ in range(self.n_perm)
                ]
...
class PCMCIPlusAdapter:
    ...
    def fit(self, df, cfg):
        ...
        results = pcmci.run_pcmciplus(
            tau_min=0, tau_max=self.tau_max, pc_alpha=self.pc_alpha
        )
...
def evaluate(output, gt):
    ...
    if output.adjacency_bin is not None:
        ...
        metrics["edge_f1_undir"] = _prf(true_u[mask], pred_u[mask])["f1"]
        if output.directed_support:
            prf = _prf(gt.adjacency[mask], output.adjacency_bin[mask])
        ...
        if (
            output.lag_support
            and output.lag_matrix is not None
            and gt.lag_matrix is not None
        ):
            errs = [
                abs(int(output.lag_matrix[s, d]) - int(gt.lag_matrix[s, d]))
                for s, d in np.argwhere(gt.adjacency == 1)
                if output.adjacency_bin[s, d] == 1
            ]
        ...
        if (
            output.sign_support
            and output.sign_matrix is not None
            and gt.sign_matrix is not None
        ):
            hits = [
                int(np.sign(output.sign_matrix[s, d]) == np.sign(gt.sign_matrix[s, d]))
                for s, d in np.argwhere(gt.adjacency == 1)
                if output.adjacency_bin[s, d] == 1
            ]
...
def run_benchmark(n_repeats=5):
    ...
    print("=" * 65)
    print("  GETTER One v0.1.9 (adaptive + partial corr) vs Baselines")
        ...
        for rep in range(n_repeats):
            ...
            for adapter in methods:
                ...
                try:
                    ...
                    m = evaluate(out, gt)
                    m["scenario"] = sname
                    m["method"] = adapter.method_name
                    m["repeat"] = rep
                    m["time_s"] = elapsed
                    all_rows.append(m)
                except Exception as e:
                    print(f"⚠️{adapter.method_name}:{e}", end=" ")
        print("✅")
    ...
    mcols = [
        ...
        if c not in ("scenario", "method", "repeat", "time_s")
    ]
    ...
    for method in summary.index:
        ...
        f1d = (
            r.get("edge_f1_dir", 0) if not np.isnan(r.get("edge_f1_dir", np.nan)) else 0
        )
        f1u = (
            r.get("edge_f1_undir", 0)
            if not np.isnan(r.get("edge_f1_undir", np.nan))
            else 0
        )
        lag = r.get("lag_mae", np.nan)
        lag_s = 1 / (1 + lag) if not np.isnan(lag) else 0
        sign = r.get("sign_acc", np.nan)
        sign_s = sign if not np.isnan(sign) else 0
        spur = r.get("spurious_rate", np.nan)
        spur_s = (1 - spur) if not np.isnan(spur) else 0.5
        c = (f1d * 1.5 + f1u * 1.0 + lag_s * 1.0 + sign_s * 0.8 + spur_s * 0.8) / 5.1
        composite[method] = c
        print(
            f"  {method:20s}: {c:.3f}  (F1d={f1d:.3f} F1u={f1u:.3f} Lag={lag_s:.3f} Sign={sign_s:.3f} Spur={spur_s:.3f} | {r['time_s']:.2f}s)"
        )
