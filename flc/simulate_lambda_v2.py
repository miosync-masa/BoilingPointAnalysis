...
def simulate_lambda_jax(schedule_dict, mat_dict, edr_dict):
    ...
    def time_step(carry, inputs):
        T, cv, rho_d, ep_eq, h_eff, eps3 = carry
        idx = inputs  # 時間インデックス
        ...
        K_fr = (2.0 * edr_dict['chi'] * q_fric) / h_eff_new
        ...
        K_total *= beta_multiplier_jax(beta_inst, edr_dict['beta_A'], edr_dict['beta_bw'])
        ...
        new_carry = (T_new, cv_new, rho_d_new, ep_eq_new, h_eff_new, eps3_new)
...
if JAX_AVAILABLE:
    ...
    def loss_single_exp_jax(schedule_np, mat, edr_dict, failed):
        ...
        # データをJAX形式に変換
        schedule_dict = schedule_to_jax_dict(schedule_np)
        ...
        Lambda = res["Lambda"]
        # 簡易スムージング（移動平均の代わり）
        Lambda_smooth = Lambda  # TODO: JAX版スムージング実装
