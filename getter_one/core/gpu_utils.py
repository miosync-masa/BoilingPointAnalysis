...
if TYPE_CHECKING:
    ...
    if HAS_GPU:
        ...
        NDArray = Union[NPArray, CPArray]  # noqa: UP007  # noqa: UP007
    else:
        NDArray = NPArray
else:
...
class GPUBackend:
    ...
    def __init__(
        ...
        profile: bool = False,
        memory_manager_config: dict[str, Any] | None = None,
    ):
        """
        Parameters
        ----------
    ...
    def _initialize_memory_manager(self, config: dict[str, Any] | None = None):
        """メモリマネージャーを初期化"""
        # インポートは実行時に（循環参照回避）
    ...
    def _select_device(self, device: str | int) -> int:
        """デバイス選択ロジック"""
        if device == "auto":
            # 自動選択（メモリが一番空いてるGPU）
    ...
    def to_gpu(self, array: NDArray, dtype: np.dtype | None = None) -> NDArray:
        """配列をGPUに転送（またはそのまま）"""
        if self.is_gpu and HAS_GPU:
